    ...
    <>
      ...
      >
        <div className="flex items-stretch h-14">
          {BOTTOM_NAV.map(({ name, href, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className="no-tap flex-1 flex flex-col items-center justify-center gap-0.5 relative active:scale-90 transition-transform"
              >
                {active && (
                  <span className="absolute top-1.5 w-5 h-0.5 rounded-full bg-orange-400" />
                )}
                <Icon
                  size={22}
                  strokeWidth={active ? 2.2 : 1.6}
                  className={`transition-all ${active ? 'text-orange-400' : 'text-zinc-500'}`}
                />
                <span className={`text-[10px] font-medium leading-none transition-all ${active ? 'text-orange-400' : 'text-zinc-600'}`}>
                  {name}
                </span>
              </Link>
            );
          ...
          <button
            onClick={() => setDrawerOpen(true)}
            className="no-tap flex-1 flex flex-col items-center justify-center gap-0.5 active:scale-90 transition-transform"
          >
            <div className="w-5 h-5 flex flex-col justify-center gap-[3.5px]">
