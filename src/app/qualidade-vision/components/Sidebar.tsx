...
import {
  ...
  ArrowRightLeft, Grid3X3, LogOut, X, ChevronRight, UserCircle,
} from 'lucide-react';
...
function pageTitle(pathname: string): string {
  ...
  for (const [key, val] of Object.entries(map)) {
    if (pathname === key || (key !== '/qualidade-vision' && pathname.startsWith(key))) return val;
  }
  return 'Quality Vision';
}

function isActive(href: string, pathname: string) {
  return pathname === href || (href !== '/qualidade-vision' && pathname.startsWith(href));
}
  ...
  return (
    <>
      ...
      <aside className="fixed top-0 bottom-0 left-0 z-40 w-64 bg-[#09090b] border-r border-white/[0.06] flex-col justify-between hidden lg:flex">
        <div>
          ...
          <nav className="p-3 space-y-0.5">
            {ALL_ITEMS.map(({ name, href, icon: Icon }) => {
        ...
        <div className="p-3 border-t border-white/[0.06] space-y-2">
          ...
          <Link href="/rh-vision/colaboradores/me" className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.05] transition-colors group">
            ...
            <div className="flex items-center gap-1">
              <UserCircle size={13} className="text-zinc-600 group-hover:text-zinc-400 transition-colors" />
              <a href="/api/auth/logout" onClick={e => e.stopPropagation()} className="p-1 text-zinc-600 hover:text-red-400 rounded-lg transition-colors" title="Logout">
                <LogOut size={14} />
              </a>
            </div>
          </Link>
      ...
      >
        ...
        >
          <div>
            <p className="text-[11px] font-bold tracking-[0.15em] uppercase text-emerald-400">Quality Vision</p>
            ...
            return (
              ...
              >
                ...
                <Icon
                  ...
                  className={`relative z-10 transition-all duration-200 ${active ? 'text-emerald-400' : 'text-zinc-600'}`}
                />
