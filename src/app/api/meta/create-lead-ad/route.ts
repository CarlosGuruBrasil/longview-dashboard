...
export async function GET(req: NextRequest) {
  ...
  const exec = new URL(req.url).searchParams.get('exec');
  if (exec !== '1') return NextResponse.json({ ok:true, token:!!TOKEN, act:ACT });

  const log: any[] = [];

  // 1. Criar 2 campanhas ABO (sem budget na campanha, budget nos adsets)
  const [rCampL, rCampR] = await Promise.all([
    post(`${ACT}/campaigns`, { name:'HUB Beira Mar | Leads | Form | Jul 2026', objective:'OUTCOME_LEADS', status:'PAUSED', buying_type:'AUCTION', special_ad_categories:[] }).catch(e => ({ data:{ error: e.response?.data?.error?.message || e.message } })),
    post(`${ACT}/campaigns`, { name:'HUB Beira Mar | Retargeting | Leads | Jul 2026', objective:'OUTCOME_LEADS', status:'PAUSED', buying_type:'AUCTION', special_ad_categories:[] }).catch(e => ({ data:{ error: e.response?.data?.error?.message || e.message } })),
  ]);

  const campL = (rCampL as any).data?.id;
  const campR = (rCampR as any).data?.id;
  log.push({ campL, campR, errL: (rCampL as any).data?.error, errR: (rCampR as any).data?.error });
  if (!campL || !campR) return NextResponse.json({ ok:false, log });

  // 2. Criar 4 adsets ABO com daily_budget
  const adsetResults = await Promise.all(ADSETS_CONFIG.map(async cfg => {
    const campId = cfg.camp === 'leads' ? campL : campR;
    try {
      const r = await post(`${ACT}/adsets`, {
        name:              cfg.name,
        campaign_id:       campId,
      ...
      return { name: cfg.name, id: null, ok: false, error: e.response?.data?.error?.error_user_msg || e.response?.data?.error?.message || e.message };
    }
  }));

  log.push({ adsets: adsetResults });
  const validAdSets = adsetResults.filter(a => a.ok && a.id);
    ...
    for (const copy of COPIES) {
      try {
        const r = await post(`${ACT}/ads`, {
          name:     `HBM | ${copy.label} | ${adSet.name.replace('HBM | ','').replace(' | ','/')}`,
          adset_id: adSet.id,
          status:   'PAUSED',
          creative: { object_story_spec: { page_id: PAGE_ID, video_data: {
            video_id: copy.video_id, image_hash: IMG_HASH, message: copy.msg, title: copy.hl,
            call_to_action: { type:'LEARN_MORE', value:{ lead_gen_form_id: FORM_ID } },
          }}},
        });
        adResults.push({ ad: `HBM|${copy.label}`, adset: adSet.name, ok:true, id:(r as any).data?.id });
      } catch (e: any) {
        const err = (e as any).response?.data?.error?.error_user_msg || (e as any).response?.data?.error?.message || (e as any).message;
        adResults.push({ ad:`HBM|${copy.label}`, adset:adSet.name, ok:false, error:err });
      }
      await new Promise(r => setTimeout(r, 400));
    }
  }

  const created = adResults.filter(r => r.ok).length;
  const failed  = adResults.filter(r => !r.ok).length;
  return NextResponse.json({ ok: failed===0 && validAdSets.length===4, campaigns:{leads:campL, retargeting:campR}, adsets:adsetResults, ads:{created,failed,results:adResults}, log });
}

export async function POST(req: NextRequest) {
  if (!auth(req)) return NextResponse.json({ error:'401' }, { status:401 });
  return NextResponse.json({ ok:true, message:'Use GET ?exec=1 para executar o fluxo completo' });
}
