...
function axiosData(err: unknown): unknown {
  return axios.isAxiosError(err) ? err.response?.data : null;
}
...
export async function GET() {
  ...
  const [campaignsRes, adsetsRes] = await Promise.allSettled([
    ...
    axios.get<MetaListResponse>(`${META_BASE}/${META_ACT_ID}/adsets`, {
      params: {
  ...
  return NextResponse.json({
    campaigns: campaignsRes.status === 'fulfilled' ? campaignsRes.value.data?.data ?? [] : [],
    adsets:    adsetsRes.status === 'fulfilled'    ? adsetsRes.value.data?.data ?? [] : [],
    errors: {
      campaigns: campaignsRes.status === 'rejected' ? axiosData(campaignsRes.reason) : null,
      adsets:    adsetsRes.status    === 'rejected' ? axiosData(adsetsRes.reason) : null,
    }
  });
}
...
export async function PATCH(request: NextRequest) {
  ...
  const body = await request.json() as MetaPatchBody;
  ...
  if (daily_budget) updatePayload.daily_budget = Math.round(parseFloat(String(daily_budget)) * 100); // centavos
  if (lifetime_budget) updatePayload.lifetime_budget = Math.round(parseFloat(String(lifetime_budget)) * 100);
